...
public:
    ...
    */
    ...
    void insert(const Key& key, const Value& value);
    //void _insert(const Key& key, const Value& value, Node* node);
    // удалить все элементы с ключем key
    void erase(const Key& key);

    // найти первый элемент в дереве, равный ключу key
    ConstIterator find(const Key& key) const; //ok
    Iterator find(const Key& key); //ok
    ...
    ConstIterator min(const Key& key) const; //ok
    // получить итератор на максимальное значение в дереве
    ConstIterator max(const Key& key) const; //ok

    Iterator begin(); //ok
    Iterator end(); //ok

    ConstIterator cbegin() const; //ok
    ConstIterator cend() const; //ok
